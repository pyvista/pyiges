...
The ``pyiges.Iges`` class stores each individual IGES entity separately
and each entity of an ``pyiges.Iges`` object can be accessed either via
indexing or by querying for a specific type of entity
...
Many of the entities are not yet fully supported, and those entities are
represented as a generic ``pyiges.geometry.Entity`` object. However,
many entities are supported, including:
